...
Vue.prototype.$layouts = {
    ...
    Arena: function() {
        return Vue.prototype.$parseLayout([
            [
                "#####...#####",
                "####.###.####",
                "###...#...###",
...
Vue.prototype.$parseLayout = function (slices) {
    let layout = [];
    for (let z = 0; z < slices.length; z++) {
        for (let y = 0; y < slices[z].length; y++) {
            slices[z][y].split("").forEach(function (c, x) {
                if (c === '#') {
                    layout.push([x, y, z]);
                } else if (c === 'v') {
                    layout.push([x, y + 0.5, z]);
                }
            });
        }
    }
